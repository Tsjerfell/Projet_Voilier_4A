...
void UART_Init ( USART_TypeDef * UARTptr,Mode_TypeDef Mode, int UART_BaudRate ) {
	...
	if (Mode== TX) { 
		UARTptr  -> CR1 |= USART_CR1_TE;
	}
		if (Mode== RX){//Enable transmitter 
		UARTptr  -> CR1 |= USART_CR1_RE; 
		}//Enable receiver 
...
void UART_ActiveIT (USART_TypeDef * UARTptr, char Prio, void (* IT_Function )(void)){
	...
	if (UARTptr  == USART1) {
		...
		NVIC->IP[num_USART] = (Prio << 4); // Fixe la priorité de l'interruption dans le NVIC
	  NVIC->ISER[1] |= NVIC_ISER_SETENA_5; } // Autorise la prise en compte de l'interruption dans le NVIC
