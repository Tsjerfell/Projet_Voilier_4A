...
void Init_Codeur (TIM_TypeDef * Timer)
	{
		Timer->CCMR1 &= 0xFCFC;  			//Initialisation du registre CCMR1
		Timer->CCMR1 |= 0x01;
		Timer->CCMR1 |= 0x01 << 8;
		Timer->CCMR1 &= 0x0F0F;
		
		Timer->CCER &= 0xFFDD;				//Initialisation du registre CCER
		
		Timer->SMCR &= (0xFFFF) << 3;				//Initialisation du registre SMCR
		Timer->SMCR |= 0x03;
